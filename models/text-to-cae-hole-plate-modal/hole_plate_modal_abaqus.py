"""Abaqus/CAE modal analysis script for the Text-to-CAE perforated plate example.

Run with:
    abaqus cae noGUI=models/text-to-cae-hole-plate-modal/hole_plate_modal_abaqus.py
"""
...
def script_root():
    ...
    cwd_candidate = os.path.join(os.getcwd(), "models", "text-to-cae-hole-plate-modal")
...
ROOT = script_root()
MODEL_NAME = "TextToCAE_HolePlate_Modal_Model"
JOB_NAME = "TextToCAE_HolePlate_Modal"
...
def apply_parameters_to_manifest(manifest):
    manifest["inputs"]["summary"] = [
        ...
        },
        {
            ...
            "value": {
                "en": "Left face fixed for constrained natural frequency extraction",
                "zh": u"\u5de6\u7aef\u9762\u56fa\u5b9a\uff0c\u63d0\u53d6\u53d7\u7ea6\u675f\u56fa\u6709\u9891\u7387",
            },
        },
        {
    ...
    manifest["inputs"]["loads"] = {
        "description": "Left face fixed; no external load; frequency extraction for the first {} modes".format(MODE_COUNT),
        "fixed_end": "x = 0 face",
        "mode_count": MODE_COUNT,
        "load_direction": "modal free vibration",
    }
...
def build_model():
    ...
    model.FrequencyStep(name="Modal", previous="Initial", numEigen=MODE_COUNT)
    model.EncastreBC(name="FixedLeft", createStepName="Initial", region=assembly.sets["left_face"])

    mdb.Job(
        name=JOB_NAME,
        model=MODEL_NAME,
        description="Text-to-CAE modal analysis of a plate with a circular hole",
        type=ANALYSIS,
        atTime=None,
        waitMinutes=0,
        waitHours=0,
...
def extract_results():
    ...
    try:
        ...
        modes = []
        for frame in modal_step.frames:
            ...
            if frequency is None:
                ...
                if marker in description:
                    try:
                        frequency = float(description.split(marker, 1)[1].split()[0])
                    except Exception:
                        frequency = 0.0
            mode_number = getattr(frame, "mode", None) or (len(modes) + 1)
            max_umag = 0.0
            ...
            modes.append({
                ...
                "frequency_hz": float(frequency or 0.0),
                "max_raw_umag": float(max_umag),
            })
    finally:
        odb.close()
    ...
    manifest = load_manifest()
    apply_parameters_to_manifest(manifest)
    manifest["project"]["name"] = "Text to CAE Plate With Circular Hole Modal Analysis"
    manifest["project"]["prompt"] = "Compute constrained natural frequencies and mode shapes for a steel plate with a central circular hole."
    manifest["project"]["model"] = "3D modal solid mechanics"
    manifest["project"]["source"] = "models/text-to-cae-hole-plate-modal/hole_plate_modal_abaqus.py"
    manifest["connection"] = {
        "status": "connected",
        "message": "Abaqus solved the modal model and extracted ODB results.",
    }
    manifest["outputs"]["status"] = "complete"
    manifest["outputs"]["job_name"] = JOB_NAME
    manifest["outputs"]["odb"] = "models/text-to-cae-hole-plate-modal/{}.odb".format(JOB_NAME)
    manifest["outputs"]["modes"] = modes
    metrics = []
    for mode in modes[:4]:
        metrics.append({
            "label": "Mode {} frequency".format(mode["mode"]),
            "value": round(float(mode["frequency_hz"]), 4),
            "unit": "Hz",
            "state": "complete",
        })
    manifest["outputs"]["metrics"] = metrics
    for item in manifest["workflow"]:
        if item["step"] == "Parse prompt":
            item["detail"] = "The natural language request is mapped to a perforated plate, steel material with density, fixed edge, and a modal frequency extraction step."
        if item["step"] == "Build Abaqus model":
            item["detail"] = "The Abaqus script creates the plate with a circular hole, material density, section, assembly, boundary condition, mesh, and modal job."
        if item["step"] == "Solve":
            item["detail"] = "Abaqus/CAE noGUI runs a frequency extraction job and writes modal frames to the ODB."
        if item["step"] == "Review results":
            item["detail"] = "The scripts extract natural frequencies and mode-shape displacement fields from the ODB."
...
def main():
    os.chdir(ROOT)
    update_status("building", message="Abaqus is building the modal CAE model.")
    build_model()
    update_status("running", message="Abaqus modal job submitted.")
    job = mdb.jobs[JOB_NAME]
    job.submit(consistencyChecking=OFF)
    job.waitForCompletion()
    extract_results()
    print("Text-to-CAE perforated plate modal analysis complete: {}".format(MANIFEST_PATH))
